...
<div id="appForm">
  ...
  <small><a href="addApplication?AppID=<?=$appId?>">Add New App</a></small>
		<font color="#cccccc">|</font> <small><strong><a href="reportingAll?AppID=<?=$appId?>">View All Apps Reporting</a></strong></small>
  </form>
  </div>
</div>
<br />
...
<?
		...
		$flashUrl = "adrolloData?nocache=$rand%26AppID=$appId%26txtStartDate=$startDate%26txtEndDate=$endDate%26viewtype=$chartType";
		$downloadUrl = "downloadData?nocache=$rand&AppID=$appId&txtStartDate=$startDate&txtEndDate=$endDate&viewtype=$chartType";
		?>
...
<select name="viewtype" id="viewtype" style="width: 170px" onchange="this.form.submit()">
  ...
  <?php 
  foreach ($allCustom as $oneCustom) {
  echo '<option value="'.$oneCustom['NID'].'" '.((''.$chartType == $oneCustom['NID']) ? 'selected' : '').'>'.$oneCustom['Name'].'</option>';
}
  ?>
  </select>
